...
{
    [self makeToast:message duration:kDefaultLength position:kDefaultPosition isShadow:YES];
}
...
{
    UIView *toast;
    toast = (UIView *)[self viewWithTag:kToastTag];
...
- (void)makeToastActivityOffsetTop{
    ...
    CGPoint point = [self getPositionFor:@"top" toast:activityContainer];
    [activityContainer setCenter:CGPointMake(point.x, point.y + 80)];
...
- (CGPoint)getPositionFor:(id)point toast:(UIView *)toast {
    ...
    if([point isKindOfClass:[NSString class]]) {
        ...
        }else if( [point caseInsensitiveCompare:@"topCenter"] == NSOrderedSame ) {
            return CGPointMake(self.bounds.size.width / 2, (toast.frame.size.height / 2)+kVerticalPadding*6);
        }
    ...
    LogE(@"Error: Invalid position for toast.");
...
- (UIView *)makeViewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image isShadow:(BOOL)_isShadow{
    ...
    if(titleLabel != nil) {
        [titleLabel setFrame:CGRectMake(messageLeft, titleTop + 10, messageWidth, titleHeight)];
